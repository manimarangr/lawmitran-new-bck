...
export const GROUPS: { id: string; title: string; description: string }[] = [
  { id: 'payments', title: 'Payments — Razorpay', description: 'Order creation and signature verification use these keys. Keep test keys until you go live.' },
  { id: 'sms', title: 'SMS gateway', description: 'OTP delivery. Off = OTPs are logged to the backend console (dev mode).' },
  { id: 'whatsapp', title: 'WhatsApp', description: 'WhatsApp OTP/notifications. Currently a placeholder integration — settings are stored for go-live.' },
  { id: 'email', title: 'Email (SMTP)', description: 'Transactional email. Unset = emails are logged to the backend console (dev mode).' },
  { id: 'captcha', title: 'reCAPTCHA', description: 'Signup bot protection. Off or unset = captcha checks are skipped.' },
  { id: 'ai', title: 'AI assistance', description: 'LLM synthesis for the homepage legal-question box. Off = curated knowledge base only (still fully functional).' },
...
export const SETTINGS_REGISTRY: SettingDef[] = [
  // payments
  { key: 'PAYMENTS_MODE', group: 'payments', label: 'Mode', type: 'select', options: ['test', 'live'] },
  { key: 'RAZORPAY_KEY_ID', group: 'payments', label: 'Key ID', type: 'text', placeholder: 'rzp_test_…' },
  { key: 'RAZORPAY_KEY_SECRET', group: 'payments', label: 'Key Secret', type: 'secret' },
  { key: 'RAZORPAY_WEBHOOK_SECRET', group: 'payments', label: 'Webhook Secret', type: 'secret' },
  { key: 'CURRENCY', group: 'payments', label: 'Currency', type: 'text', placeholder: 'INR' },
  // sms
  { key: 'SMS_ENABLED', group: 'sms', label: 'Enable SMS sending', type: 'toggle' },
  { key: 'SMS_PROVIDER', group: 'sms', label: 'Provider', type: 'select', options: ['msg91', 'twilio', 'textlocal'] },
  { key: 'SMS_API_KEY', group: 'sms', label: 'API key', type: 'secret' },
  { key: 'SMS_SENDER_ID', group: 'sms', label: 'Sender ID', type: 'text', placeholder: 'LWMTRN' },
  { key: 'SMS_OTP_TEMPLATE_ID', group: 'sms', label: 'OTP template ID', type: 'text' },
  // whatsapp
  { key: 'WHATSAPP_ENABLED', group: 'whatsapp', label: 'Enable WhatsApp sending', type: 'toggle' },
  { key: 'WHATSAPP_TOKEN', group: 'whatsapp', label: 'Access token', type: 'secret' },
  { key: 'WHATSAPP_PHONE_NUMBER_ID', group: 'whatsapp', label: 'Phone number ID', type: 'text' },
  { key: 'WHATSAPP_OTP_TEMPLATE', group: 'whatsapp', label: 'OTP template name', type: 'text' },
  // email
  { key: 'SMTP_HOST', group: 'email', label: 'SMTP host', type: 'text', placeholder: 'smtp.zoho.in' },
  { key: 'SMTP_PORT', group: 'email', label: 'SMTP port', type: 'number', placeholder: '587' },
  { key: 'SMTP_USER', group: 'email', label: 'SMTP user', type: 'text' },
  { key: 'SMTP_PASS', group: 'email', label: 'SMTP password', type: 'secret' },
  { key: 'MAIL_FROM', group: 'email', label: 'From address', type: 'text', placeholder: 'LawMitran <no-reply@lawmitran.com>' },
  { key: 'SUPPORT_EMAIL', group: 'email', label: 'Support inbox', type: 'text', placeholder: 'support@lawmitran.com', help: 'Client queries are notified here.' },
  // captcha
  { key: 'RECAPTCHA_ENABLED', group: 'captcha', label: 'Enable reCAPTCHA', type: 'toggle' },
  { key: 'RECAPTCHA_SITE_KEY', group: 'captcha', label: 'Site key', type: 'text' },
  { key: 'RECAPTCHA_SECRET_KEY', group: 'captcha', label: 'Secret key', type: 'secret' },
  // ai assistance
  { key: 'AI_ENABLED', group: 'ai', label: 'Enable LLM synthesis', type: 'toggle' },
  { key: 'AI_PROVIDER', group: 'ai', label: 'Provider', type: 'select', options: ['openai', 'gemini'] },
  ...
  // billing
  { key: 'GSTIN', group: 'billing', label: 'GSTIN', type: 'text', placeholder: '29ABCDE1234F1Z5' },
  { key: 'GST_LEGAL_NAME', group: 'billing', label: 'Legal entity name', type: 'text', placeholder: 'LawMitran Technologies Pvt Ltd' },
  { key: 'GST_ADDRESS', group: 'billing', label: 'Registered address', type: 'text' },
  { key: 'GST_RATE', group: 'billing', label: 'GST rate (%)', type: 'number', placeholder: '18' },
  { key: 'INVOICE_PREFIX', group: 'billing', label: 'Invoice prefix', type: 'text', placeholder: 'LM' },
  // business
