...
function SignupFormInner({ role, heading, subheading, submitLabel }: SignupFormProps) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold text-navy">{heading}</h1>
      <p className="mb-6 mt-1 text-sm text-slate-500">{subheading}</p>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"
            >
              <Icon name={showConfirmPw ? 'eye-slash' : 'eye'} className="text-sm" />
            </button>
          </div>
          {errors.confirmPassword && (
            <p id="confirmPassword-error" role="alert" className="mt-1 text-xs text-rose-600">
