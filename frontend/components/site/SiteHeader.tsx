    ...
    <header className="sticky top-0 z-50 border-b border-line bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex h-[74px] max-w-[1180px] items-center justify-between px-5">
        <Link href="/" className="flex items-center" aria-label="LawMitran home">
          <Image src="/logo.svg" alt="LawMitran" width={160} height={38} className="h-[38px] w-auto" priority />
        </Link>
        ...
        <nav id="mobile-nav" aria-label="Main menu" className="border-t border-line bg-white px-5 py-4 md:hidden">
          <ul className="space-y-1">
            ...
            {me ? (
              <>
                <li>
                  <Link href={dashboardPath(me.role)} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2.5 text-[15px] font-semibold text-ink hover:bg-bg-soft">
                    {me.role === 'ADMIN' ? 'Admin console' : 'Dashboard'}
                  </Link>
                </li>
                <li>
                  <button onClick={() => { setOpen(false); signOut(); }} className="block w-full rounded-lg px-3 py-2.5 text-left text-[15px] font-semibold text-rose-600 hover:bg-rose-50">
                    Sign out
                  </button>
                </li>
              </>
              ...
              <>
                <li>
                  <Link href="/login" onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2.5 text-[15px] font-semibold text-ink hover:bg-bg-soft">
                    Login
                  </Link>
                </li>
                <li>
